...
def signup_page(request):
    if request.method == 'POST':
        ...
        #补充域名或ip
        path = "https://" + reverse('active') + "?token=" + token
        msg = f"""
        <!DOCTYPE html>
        <html>
        <body>
        ...
        return render(request, 'signup.html', {'error_message': "邮件已发送，请访问邮件链接激活账户"})
    else:
        form = SignUpForm()
...
def active(request):
    ...
    user.is_active = True
    user.save()
    return render(request, "index.html", {"error_message": "激活成功，请转到登陆页面登录"})    
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if user.is_active == True:
                ...
                request.session.save()
                #补充域名或ip
                path = "https://" + reverse('reset') + "?token=" + token
                msg = f"""
                <!DOCTYPE html>
                <html>
                <body>
