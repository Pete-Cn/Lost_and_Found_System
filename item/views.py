...
def item(request):
    if request.method == "POST":
        ...
        Item_list = Item_list.filter(is_hidden=False)
...
def detail(request, item_id): 
    try:
        current_item = Item.objects.get(pk=int(item_id))
        if current_item.is_hidden == False or request.user.is_staff == True:
            if request.method == "POST":
            ...
            return HttpResponse("您所查找的页面不存在")
    except:
        return HttpResponse("您所查找的页面不存在")
...
def deleteItem(request, item_id):
    try:
        current_item = Item.objects.get(pk=int(item_id))
        if current_item.founder == request.user.username or request.user.is_staff == True:
            current_item.is_hidden = True
            current_item.save()
            return HttpResponseRedirect(reverse("items:detail", kwargs={'item_id': item_id}))
        else:
            return HttpResponse("无权删除此页面")
    except:
        return HttpResponse("删除的物品不存在")
...
def search(request):
    ...
    qualified_items = Item.objects.filter(q & q3 & q2)
    qualified_items = qualified_items.filter(is_hidden=False)
...
def loaddata(request):
    if request.user.is_staff == True:
        ...
        return HttpResponseRedirect(request, "index.html", {'error_message': "您无权查看此页面"})
